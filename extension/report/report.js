...
export function createReportHtml(reportData, iconDataURL = null) {
  console.log('[report] createReportHtml called', reportData);
  ...
  function card(title, contentHtml, gradient = 'linear-gradient(to bottom right, #2563eb, #16a34a, #000000)') {
    return `<div class="card-wrapper">
      <div class="card-inner" style="background-image: ${gradient};">
  ...
  `, 'linear-gradient(to bottom right, #1DB954, #0A66C2'));
  ...
  const iconHtml = iconDataURL
    ? `<img src="${escapeHtml(iconDataURL)}" alt="LinkedIn Wrapped 2025" class="header-icon" />`
    : '';
  ...
  <head>
    ...
    <title>Your LinkedIn Wrapped 2025</title>
    <style>
    ...
    <div class="container">
      <div class="header">
        ...
        <div class="header-content">
          <div class="header-title">Your LinkedIn Wrapped 2025</div>
      ...
      <div class="actions">
        ...
        <button id="exportAll" class="btn">📸 Export as Image</button>
      </div>
    </div>
...
if (typeof window !== 'undefined') {
  window.__TEST_createReportHtml = (data) => {
    try { const out = createReportHtml(data); console.log('[report] html length', out.length); return out; }
    catch (e) { console.error('[report] test error', e); return null; }
  };
}